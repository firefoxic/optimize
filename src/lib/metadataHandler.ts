...
export class MetadataHandler {
	...
	public metadata: Map<string, ImageMetadata>
	 ...
	 */
	public static init (options: BaseOptions): MetadataHandler | null {
	 ...
	 */
	public prepare (imageName: string): boolean {
	 ...
	 */
	public addSizeInfo (imageName: string, width: number, height: number, breakpointMatch: RegExpMatchArray | null): void {
		...
		if (breakpointMatch && breakpointMatch[2]) sizeInfo.breakpoint = +breakpointMatch[3]
	 ...
	 * @param {string} imageName - Name of the image.
	 */
	public finalizeMetadata (imageName: string): void {
		if (!this.options.addMetaData) return
	 ...
	 */
	public async writeDataJson (): Promise<void> {
