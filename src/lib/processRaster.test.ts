...
vi.mock(`../../src/lib/metadataHandler.js`, async (importOriginal) => {
	let actual = await importOriginal() as Record<string, unknown>
	return {
		...actual,
		MetadataHandler: {
			init: vi.fn(() => null),
...
describe(`processRaster`, () => {
	...
	it(`should process raster files with density 0`, async () => {
		let { convertToFormats } = await import(`./convertToFormats.js`)
	...
	it(`should process multiple target formats`, async () => {
		...
		let options = {
			...
			inputDirectory: testDir,
			outputDirectory: testDir,
			targetFormats: [`avif`, `webp`, `jpg`] as TargetFormat[],
			originDensity: 1,
			removeOrigin: false,
			addMetaData: false,
		}
